...
try:
    if os.path.exists(dll_path):
        ...
        if not dll.GetDevicesCount(ctypes.byref(cnt)):
            raise RuntimeError("GetDevicesCount failed")
        ...
        if cnt.value > 0:
            ...
            if not dll.GetDevicePaths(Paths):
                raise RuntimeError("GetDevicePaths failed")
    ...
    print(f"[WARN] SDK initialization failed: {_dll_err}. Falling back to UVC-only mode.")
    dll = None
    has_af = False
    cam_ids = []
...
ACTIVE_SDK_DEVICE_PATHS = cam_ids[:2] if cam_ids else []
...
def main():
    ...
    try:
        while True:
            ...
            if DEBUG_PROBES and state["calibrating"] and now >= next_probe_t:
            ...
            # Stack vertically with preserved aspect ratio (align widths once)
...
if __name__ == "__main__":
    try:
        # Prefer modular app if available
        sys.path.append(os.path.dirname(os.path.abspath(__file__)))
        from app import main as _main
        _main()
    except Exception as _delegate_err:
        # Fallback to legacy inline main
        print("[WARN] Modular app failed (", _delegate_err, ") — falling back to legacy main()")
        main()
